...
def find_prizepicks_nba_matches():
    current_date = datetime.now().strftime("%Y-%m-%d")
    try:
        df1 = pd.read_csv(f'odds-lines-data/betonline/nba/nba_betonline_odds_{current_date}.csv')
        df2 = pd.read_csv(f'odds-lines-data/prizepicks-lines/nba/prizepicks_nba_lines_{current_date}.csv')
    except KeyError as e:
        print(f"Missing column: {e}")
        exit()
    matching_lines = []
    ...
    for index, row1 in df1.iterrows():
        ...
        if not matching_row.empty:
            # Iterate over the properties
            for prop in properties:
                if prop + '_Line' in row1 and prop in matching_row:
                    prop_line_df1 = row1[prop + '_Line']
                    prop_line_df2 = matching_row[prop].iloc[0]
...
def find_prizepicks_nhl_matches():
    current_date = datetime.now().strftime("%Y-%m-%d")
    try:
        df1 = pd.read_csv(f'odds-lines-data/betonline/nhl/nhl_betonline_odds_{current_date}.csv')
        df2 = pd.read_csv(f'odds-lines-data/prizepicks-lines/nhl/prizepicks_nhl_lines_{current_date}.csv')
    except KeyError as e:
        print(f"Missing column: {e}")
        exit()
    matching_lines = []
...
def find_underdog_matches_mlb():
    current_date = datetime.now().strftime("%Y-%m-%d")
    try:
        df1 = pd.read_csv(f'odds-lines-data/betonline/mlb/mlb_betonline_odds_{current_date}.csv')
        df2 = pd.read_csv(f'odds-lines-data/underdog-lines/mlb/mlb_underdog_lines_{current_date}.csv')
    except KeyError as e:
        print(f"Missing column: {e}")
        exit()
    matching_lines = []
    properties = ["Hits", "TotalBases", "RBIs", "Runs", "Singles", "Doubles", "RunsAllowed", "HitsAllowed", "PitcherStrikeouts"]
    for prop in properties:
        prop_line_betonline = f'{prop}_Line' # 'Shots_Line', 'Points_Line', etc.
        # Iterate over each row in the betonline DataFrame
        for index, row1 in df1.iterrows():
...
def find_prizepicks_mlb_matches():
    current_date = datetime.now().strftime("%Y-%m-%d")
    try:
        df1 = pd.read_csv(f'odds-lines-data/betonline/mlb/mlb_betonline_odds_{current_date}.csv')
        df2 = pd.read_csv(f'odds-lines-data/prizepicks-lines/mlb/prizepicks_mlb_lines_{current_date}.csv')
    except KeyError as e:
        print(f"Missing column: {e}")
        exit()
    matching_lines = []
    ...
    matching_lines = sorted(matching_lines, key=lambda k: k['odds'])
    matching_lines = [line for line in matching_lines if line['odds'] <= -125]
    with open(f'odds-lines-data/matching-lines/mlb/mlb_matching_lines_{current_date}.txt', 'w') as matching_lines_file:
        json.dump(matching_lines, matching_lines_file, indent=2)
    return matching_lines

if __name__ == "__main__":
    data = []
    data += find_prizepicks_mlb_matches()
    data += find_prizepicks_nba_matches()
    data += find_prizepicks_nhl_matches()